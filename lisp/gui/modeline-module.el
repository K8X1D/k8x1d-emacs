(use-package emacs
  ...
  (defsubst k8x1d-evil-module ()
	...
	(cond
	 ...
	 ((evil-emacs-state-p) (propertize (format "%s%s%s" left-separator "EMACS" right-separator) 'face `(:foreground ,(doom-color 'magenta) :background ,default-background-color)))
	 ;; ((evil-insert-state-p) (propertize  (format "%s" "🅘") 'face `(:foreground ,(doom-color 'yellow))))
	 ((evil-insert-state-p) (propertize (format "%s%s%s" left-separator "INSERT" right-separator) 'face `(:foreground ,(doom-color 'yellow) :background ,default-background-color)))
	 ;; ((evil-motion-state-p) "🅜")
	 ((evil-motion-state-p) (propertize (format "%s%s%s" left-separator "MOTION" right-separator) 'face `(:foreground ,(doom-color 'blue) :background ,default-background-color)))
	 ;; ((evil-visual-state-p) (propertize  (format "%s" "🅥") 'face `(:foreground ,(doom-color 'red))))
	 ((evil-visual-state-p) (propertize (format "%s%s%s" left-separator "VISUAL" right-separator) 'face `(:foreground ,(doom-color 'red) :background ,default-background-color)))
	 ;; ((evil-operator-state-p) "🅞")
	 ((evil-operator-state-p) (propertize (format "%s%s%s" left-separator "OPERATOR" right-separator) 'face `(:foreground ,(doom-color 'grey) :background ,default-background-color)))
	 ;; ((evil-replace-state-p) "🅡")
	 ((evil-replace-state-p) (propertize (format "%s%s%s" left-separator "REPLACE" right-separator) 'face `(:foreground ,(doom-color 'purple) :background ,default-background-color)))
	 ;; (t (propertize  (format "%s" "🅝") 'face `(:foreground ,(doom-color 'green))))
	 (t (propertize (format "%s%s%s" left-separator "N" right-separator) 'face `(:foreground ,default-foreground-color :background ,default-background-color)))
	 ))
      ))
